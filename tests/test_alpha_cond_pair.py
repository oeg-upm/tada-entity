...
class AlphaCondPairTest(TestCase):
    ...
    def test_get_file_acc(self):
        ...
        for fsid in range(1, 6):
            for a_attr in ['mean', 'median']:
                if acc[fsid][a_attr] < 1:
                    print("TESTING fsid %d = %f" % (fsid, acc[fsid][a_attr]))
                self.assertEqual(acc[fsid][a_attr], 1)
    ...
    def test_get_acc_per_class(self):
        ...
        for fsid in range(5, 6):
            for class_idx in range(len(dfs)):
                self.assertIn(fsid, acc[class_idx])
                for a_attr in acc[class_idx][fsid]:
                    print("TEST class: %s (acc: %f)" % (correct_class_uris[class_idx], acc[class_idx][fsid][a_attr]))
    ...
    def test_workflow_swimmers(self):
        # df = pd.read_csv('tests/alpha_test_5.csv')
        # df[["colid"]] = df[["colid"]].apply(pd.to_numeric)
        # # print(df['colid'])
        # acp.add_alpha_per_file(df)
    ...
    def test_workflow2(self):
        ...
        for a_attr in ['mean', 'median']:
            # print("Swimmer should be -1")
            print(acc[correct_class_uris[0]][2][a_attr])
            self.assertEqual(acc[correct_class_uris[0]][2][a_attr], -1)

        for fsid in range(3, 5):
            self.assertIn(fsid, acc[correct_class_uris[0]])
            for a_attr in ['mean', 'median']:
                # print("fsid: %d, class: %s, attr: %s, acc: %f" % (fsid, correct_class_uris[0], a_attr, acc[correct_class_uris[0]][fsid][a_attr]))
        ...
        for fsid in range(3, 5):
            ...
            for a_attr in ['mean', 'median']:
                print("fsid: %d, class: %s, attr: %s, acc: %f" % (
                fsid, correct_class_uris[1], a_attr, acc[correct_class_uris[1]][fsid][a_attr]))
                self.assertEqual(acc[correct_class_uris[1]][fsid][a_attr], 1.0)

        for a_attr in ['mean', 'median']:
            print("fsid: %d, class: %s, attr: %s, acc: %f" % (
                fsid, correct_class_uris[1], a_attr, acc[correct_class_uris[1]][2][a_attr]))
            self.assertEqual(acc[correct_class_uris[1]][2][a_attr], -1)

        for a_attr in ['mean', 'median']:
            print("fsid: %d, class: %s, attr: %s, acc: %f" % (
                fsid, correct_class_uris[1], a_attr, acc[correct_class_uris[1]][1][a_attr]))
        ...
        for fsid in range(1, 5):
            ...
            for a_attr in ['mean', 'median']:
                print("fsid: %d, class: %s, attr: %s, acc: %f" % (
                fsid, correct_class_uris[2], a_attr, acc[correct_class_uris[2]][fsid][a_attr]))
